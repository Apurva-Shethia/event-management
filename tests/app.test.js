...
describe('User Authentication', () => {
  ...
  test('POST /register - normalizes role, email, and name fields', async () => {
    const res = await request(app).post('/register').send({
      name: '  Charlie  ',
  ...
  test('POST /register - treats email as case-insensitive for duplicates', async () => {
    await request(app).post('/register').send({
      name: 'Alice',
      email: 'ALICE@EXAMPLE.COM',
      password: 'password123',
  ...
  test('POST /login - allows case-insensitive email login', async () => {
    await request(app).post('/register').send({
      name: 'Alice',
      email: 'alice@example.com',
      password: 'password123',
...
describe('Event Management', () => {
  ...
  test('POST /events - returns 400 for invalid date format', async () => {
    const res = await request(app)
      .post('/events')
      .set('Authorization', `Bearer ${organizerToken}`)
      .send({
  ...
  test('POST /events - returns 400 for invalid time format', async () => {
    const res = await request(app)
      .post('/events')
      .set('Authorization', `Bearer ${organizerToken}`)
      .send({
  ...
  test('PUT /events/:id - returns 400 for invalid update payload', async () => {
    const createRes = await request(app)
      .post('/events')
      .set('Authorization', `Bearer ${organizerToken}`)
      .send({
  ...
  test('PUT /events/:id - returns 400 for invalid date update', async () => {
    const createRes = await request(app)
      .post('/events')
      .set('Authorization', `Bearer ${organizerToken}`)
      .send({
    ...
    const res = await request(app)
      .put(`/events/${eventId}`)
      .set('Authorization', `Bearer ${organizerToken}`)
      .send({ date: '06/15/2026' });

    expect(res.status).toBe(400);
  });

  test('PUT /events/:id - organizer cannot update another organizer event', async () => {
    const createRes = await request(app)
      .post('/events')
      .set('Authorization', `Bearer ${organizerToken}`)
      .send({
    ...
    const otherOrganizerLogin = await request(app)
      .post('/login')
      .send({ email: 'other-organizer@example.com', password: 'password123' });

    const res = await request(app)
      .put(`/events/${eventId}`)
      .set('Authorization', `Bearer ${otherOrganizerLogin.body.token}`)
      .send({ title: 'Hijacked Conference' });
  ...
  test('DELETE /events/:id - organizer cannot delete another organizer event', async () => {
    const createRes = await request(app)
      .post('/events')
      .set('Authorization', `Bearer ${organizerToken}`)
      .send({
...
describe('Participant Management', () => {
  ...
  test('GET /events/registrations/me - lists attendee registrations', async () => {
    await request(app)
      .post(`/events/${eventId}/register`)
      .set('Authorization', `Bearer ${attendeeToken}`);
    ...
    expect(res.body.registrations[0].id).toBe(eventId);
  });

  test('GET /events/registrations/me - returns empty array when no registrations', async () => {
    const res = await request(app)
      .get('/events/registrations/me')
      .set('Authorization', `Bearer ${attendeeToken}`);
  ...
  test('GET /events/registrations/me - returns 401 without token', async () => {
    const res = await request(app).get('/events/registrations/me');
    expect(res.status).toBe(401);
  });

  test('DELETE /events/:id/register - attendee can cancel registration', async () => {
    await request(app)
      .post(`/events/${eventId}/register`)
      .set('Authorization', `Bearer ${attendeeToken}`);
    ...
    expect(res.status).toBe(200);
    expect(res.body.event.participants.length).toBe(0);
  });

  test('DELETE /events/:id/register - returns 409 when attendee is not registered', async () => {
    const res = await request(app)
      .delete(`/events/${eventId}/register`)
      .set('Authorization', `Bearer ${attendeeToken}`);
  ...
  test('DELETE /events/:id/register - returns 404 for non-existent event', async () => {
    const res = await request(app)
      .delete('/events/non-existent-id/register')
      .set('Authorization', `Bearer ${attendeeToken}`);

    expect(res.status).toBe(404);
  });

  test('DELETE /events/:id/register - returns 401 without token', async () => {
    const res = await request(app).delete(`/events/${eventId}/register`);
    expect(res.status).toBe(401);
  });
});
